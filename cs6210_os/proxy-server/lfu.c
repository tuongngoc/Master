...
void evict_cache_action(size_t size_needed) {
  ...
  while ((sum_caches_entries_used + size_needed) > max_capacity){
      ...
      /*delete LFU item from index priority queue*/
...
void* gtcache_get(char *key, size_t *val_size){
  ...
  if (item != NULL ) {   /*found it from cache*/
    ...
    (*entry->num_hits) += 1;
    /*increase #of hits and update the index priority queue*/
    indexminpq_increasekey(index_priority_queue,entry->id,(indexminpq_key)entry->num_hits);
...
int gtcache_set(char *key, void *value, size_t val_size){
  ...
  if (item != NULL) { /* It's a hit*/
    ...
    entry->num_hits = malloc(sizeof(int*));
    *entry->num_hits = 1;
  } 
  ...
  if (new_key) {
    ...
    indexminpq_insert(index_priority_queue,entry->id,entry->num_hits);
  }
  else {
    indexminpq_changekey(index_priority_queue,entry->id,entry->num_hits);
  }
